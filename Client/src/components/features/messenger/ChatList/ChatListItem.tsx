...
export const ChatListItem = ({ chat, isSelected, onSelect }: ChatListItemProps) => {
  ...
  const time = new Date(chat.lastMessage?.sentAt || chat.createdAt).toLocaleTimeString([], {
    hour: '2-digit',
    minute: '2-digit',
  });
    ...
    >
      <div className="p-3">
        <div className="flex items-center gap-3">
          <Avatar src={chat.avatarUrl} name={chat.name || 'Chat'} className="h-11 w-11" />
          <div className="flex-1 min-w-0">
            <div className="flex justify-between items-center">
              <h3 className="text-sm font-medium truncate">{chat.name || 'New Chat'}</h3>
